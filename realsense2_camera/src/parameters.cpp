...
void BaseRealSenseNode::getParameters()
{
    ...
    _camera_namespace = _parameters->setParam<std::string>(param_name, "uav1");
    ...
    _base_frame_id = (static_cast<std::ostringstream&&>(std::ostringstream() << _camera_namespace << "/" << _camera_name << "/" << _base_frame_id)).str();
    _parameters_names.push_back(param_name);
